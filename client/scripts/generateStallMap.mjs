...
const coordsPath = path.join(projectRoot, 'src', 'utils', 'coords_dict.js');
const orientationPath = path.join(projectRoot, 'src', 'utils', 'orientation.js');
const outputPath = path.join(projectRoot, 'src', 'utils', 'stallMap.js');
...
const { SVG_STALL_COORDS } = await loadModule(coordsPath);
let STALL_ORIENTATION = { meat: {}, fish: {}, veggies: {} };
try {
  const orientationModule = await loadModule(orientationPath);
...
function parseFilename(filename) {
  const name = filename.replace(/\.(jpe?g)$/i, '').trim();
