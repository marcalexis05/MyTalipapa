...
export default function ContractorLogs() {
  ...
  const fetchLogs = async (silent = false) => {
    ...
    try {
      ...
      const res = await fetch('/api/contractor/activity-logs', {
    ...
    <div className="flex flex-col min-h-screen bg-[#f5f5f0] pb-20 md:pb-0">
      ...
      <main className="flex-1 px-4 md:px-6 py-5 max-w-5xl mx-auto w-full">
        ...
        <div className="flex items-center justify-between mb-5 gap-3">
          <div>
            <h1 className="text-xl md:text-2xl font-extrabold text-gray-900">My Activity Logs</h1>
            <p className="text-xs text-gray-400 mt-0.5">Your application approvals &amp; decisions</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <p className="text-xs text-gray-300">
                {search ? 'Try a different search term' : 'Your actions (approvals/rejections) will appear here'}
              </p>
            </div>
          ) : (
